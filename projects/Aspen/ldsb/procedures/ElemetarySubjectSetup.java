...
public class ElementarySubjectSetup extends ProcedureJavaSource 
{
    ...
    protected void execute() throws Exception 
    {
        ...
        masterScheduleList.forEach(mst -> 
        {
            ...
            logMessage(mst.getCourseView() + " - Grade Levels : " + gradeLevels.toString() + " Students : " + studentList.size());            
								...
								if (!schoolCourse.getNumber().contains("0060"))
								{
									studentList.forEach(std ->
									{
										if (std.getGradeLevel().equals(grdLvl))
										{
											...
											StudentSchedule ssc =  new StudentSchedule(m_modelBroker.getPersistenceKey());
    ...
    private List<String> getSections() 
	{
		...
		mstCriteria.addEqualTo(SisBeanPaths.SCHEDULE_MASTER.schedule().districtContextOid().getPath(), m_currentContext);	
        mstCriteria.addEqualTo(SisBeanPaths.SCHEDULE_MASTER.schedule().schoolOid().getPath(), m_schoolOid);	
		
		//logMessage("get sections:  SisBeanPaths.SCHEDULE_MASTER.schedule().schoolOid().getPath" + SisBeanPaths.SCHEDULE_MASTER.schedule().schoolOid().getPath() + m_currentContext + " context: " + m_currentContext);
    ...
    private Map<String, Collection<SchoolCourse>> getSchoolCourses() 
    {
        ...
        Map<String, Collection<SchoolCourse>> schoolCoursesGradeMap = getBroker().getGroupedCollectionByQuery(query, SchoolCourse.COL_GRADE_LEVEL, 100);
    ...
    private List<Student> getStudents(String mstOid) 
    {
        ...
        SubQuery scheduleSubQuery = new SubQuery(StudentSchedule.class, StudentSchedule.COL_STUDENT_OID, scheduleCriteria, true);
