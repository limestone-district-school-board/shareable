			...
			for (StudentAssessment studentAssessment : query) {
				...
				try {
					...
					case ASD_OID_GR1_MID:
						...
						isAccuracyCalc = true;
						orfAccuracy = Math.round(100 * ((float) orfWc / (orfWc + orfErr)));
						rscScore = nwfCls + nwfWwr + orfWc + getDiff(m_gradeOneMidPrecisionMap, (int) orfAccuracy);
						break;
					...
					case ASD_OID_GR2_BEG:
						...
						isAccuracyCalc = true;
						orfAccuracy = Math.round(100 * ((float) orfWc / (orfWc + orfErr)));
						accuracyPercentage = getDiff(m_gradeOneEndPrecisionMap, (int) orfAccuracy);
					...
					case ASD_OID_GR2_MID_END:
						...
						isAccuracyCalc = true;
						orfAccuracy = Math.round(100 * ((float) orfWc / (orfWc + orfErr)));
						rscScore = orfWc + getDiff(m_gradeTwoMidEndPrecisionMap, (int) orfAccuracy) + (retell * 2);
						break;
					...
					case ASD_OID_GR3_GR6:
						...
						isAccuracyCalc = true;
						isMazeCalc = true;
						orfAccuracy = (int) Math.ceil(100 * ((float) orfWc / (orfWc + orfErr)));
					...
					if (isAccuracyCalc) {
						String orfAccuracyAsString = StringUtils.leftPad(String.valueOf((int) orfAccuracy), 3, "0");
						studentAssessment.setFieldA007(orfAccuracyAsString);
					}

					if (isMazeCalc) {
						String mazeAdjustedScoreAsString = StringUtils.leftPad(String.valueOf(mazeAdjustedScore), 3,
								"0");
