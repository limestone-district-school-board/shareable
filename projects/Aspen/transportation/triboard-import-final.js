...
fs.readFile('C:/imports/transportation/CustomStudentExportLDSB.csv', 'utf-8', (err, data) => {
  if (err) throw err;
  
  const InboundWriteStream = fs.createWriteStream('\\\\10.227.4.100\\c$\\SFTP-Root\\Aspen\\triboard_import_file_inbound.csv');
  const OutboundWriteStream = fs.createWriteStream('\\\\10.227.4.100\\c$\\SFTP-Root\\Aspen\\triboard_import_file_outbound.csv');
  ...
  for (const row of rows) {
	...
	//if(secondCell != '599271')
	//	continue;
	
	//Convert inbound dates
	if(cells[9] !== null && cells[9] !== undefined && cells[9] !== '' && cells[9].toString().length > 2)
		convertedInboundStartDate = convertDate(cells[9]);
	else
		convertedInboundStartDate = '09/06/2022';
	
	if(cells[10] !== null && cells[10] !== undefined && cells[10] !== '' && cells[10].toString().length > 2)
		convertedInboundEndDate = convertDate(cells[10]);
	else
		convertedInboundEndDate = '';
	
	// Convert outbound dates
	if(cells[18] !== null && cells[18] !== undefined && cells[18] !== '' && cells[18].toString().length > 2)
		convertedOutboundStartDate = convertDate(cells[18]);
	else
		convertedOutboundStartDate = '09/06/2022';
	
	if(cells[19] !== null && cells[19] !== undefined && cells[19] !== '' && cells[19].toString().length > 2) {
		convertedOutboundEndDate = convertDate(cells[19]);
	}
	else
		convertedOutboundEndDate = '';
	
	// patch for now for larger remarks values. If > 50 do not write
	
	let inbound_remark = cells[6];
	//if(inbound_remark !== null && inbound_remark !== undefined && inbound_remark !== '' && inbound_remark.length > 49)
	//	inbound_remark = inbound_remark.substring(0, 49);
	
	let outbound_remark = cells[15];
	//if(outbound_remark !== null && outbound_remark !== undefined && outbound_remark !== '' && outbound_remark.length > 49)
	//	outbound_remark = outbound_remark.substring(0, 49);
	
	//console.log(convertedOutboundEndDate);
	...
	if(inboundFlag) {
		...
		+ cells[2] + ',' + cells[8] + ',' +  cells[4] + ',' + cells[5] + ',' + inbound_remark +
	...
	if(outboundFlag) {
		...
		+ cells[11] + ',' + cells[17] + ',' +  cells[13] + ',' + cells[14] + ',' + outbound_remark +
