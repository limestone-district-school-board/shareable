		...
		const rows = data.split('\n');
		  ...
		  for (const row of rows) {
			...
			if((cells[4] === null || cells[4] === undefined || cells[4] === ""))
				continue;
			if((cells[17] === null || cells[4] === undefined || cells[4] === ""))
				continue;
			
			// only process row if current date matches either create date or date last updated
			var currentDateMinus3 = new Date();
			currentDateMinus3.setDate(currentDateMinus3.getDate() - 1);
			...
			+ " Modified: " + modifiedDate.getTime());
			 ...
			 if (modifiedDate.getTime() < currentDateMinus3.getTime()) {
				continue;
			 }
					...
					if(final_score != 0) {
						...
						**/
						...
						if(final_score <= rc_benchmark_score_0306) {
							...
							aspenImportAlertWriteStream.write(studentOID + ',2,' + 'alertIcons/cl_study.png' + ',02/02/2023' + ',08/30/2028,0,' + 
